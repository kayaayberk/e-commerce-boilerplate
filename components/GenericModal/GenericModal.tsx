...
export function GenericModal({ open, onOpenChange, title, children, className }: FacetsModalProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn('max-w-[90%] content-start bg-white sm:max-w-[500px] rounded-2xl', className)}
      >
        <DialogHeader>{!!title && <DialogTitle className='tracking-wide'>{title}</DialogTitle>}</DialogHeader>
        {children}
