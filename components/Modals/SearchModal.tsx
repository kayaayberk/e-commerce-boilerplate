...
import { type Modal, useModalStore } from '@/lib/stores/modalStore'
    ...
    case 'loading':
      return (
        <div className='flex w-full items-center justify-center'>
          <Icons.Spinner className='size-10 border-black border-b-white' />
        </div>
      )
    case 'error':
      return (
            ...
            <ul className='mb-6 flex max-w-72 flex-col gap-1 text-[20px]'>
              {results.slice(0, 5).map((singleResult) => (
                <Link key={singleResult.id} href={`/products/${singleResult.handle}`}>
                  <li onClick={() => closeModal('search')}>
                    {getHighlightedText(singleResult.title, query)}
                  </li>
                </Link>
              ))}
            </ul>
